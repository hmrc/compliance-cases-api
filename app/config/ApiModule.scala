...
import controllers.ComplianceApiController
...
class ApiModule extends AbstractModule {
  ...
  lazy val connectorBindings: Unit = {
    bind(classOf[ComplianceCasesConnector]).asEagerSingleton()
  }

  lazy val controllerBindings: Unit = {
    bind(classOf[ComplianceApiController]).asEagerSingleton()
  ...
  lazy val serviceBindings: Unit = {
    bind(classOf[ComplianceCasesService]).asEagerSingleton()
